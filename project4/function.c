...
void printShape(struct Matrix matrix)
{
    if(matrix.content==NULL)
    {
...
void reshapeMatrix(struct Matrix * mpointer,long long row,long long column)
{
    if(mpointer->content==NULL)
    {
...
struct Matrix transposeMatrix(struct Matrix matrix)
{
    ...
    {
        float * mpointer=(float *)malloc(4*matrix.row*matrix.column);
        ...
        for(long long rowpos=0;rowpos<row;rowpos++)
        {
            ...
            {
                mpointer[rowpos*col+colpos]=matrix.content[rowpos+colpos*matrix.column];
            }
        }
...
struct Matrix matmul_plain(struct Matrix matrix1,struct Matrix matrix2)
{
    if(matrix1.content==NULL||matrix2.content==NULL)
    {
...
struct Matrix matmul_improved_1(struct Matrix matrix1,struct Matrix matrix2)
{
    if(matrix1.content==NULL||matrix2.content==NULL)
    {
        printf("The matrix you multiply doesn't exist!\n");
...
struct Matrix matmul_improved_2(struct Matrix matrix1,struct Matrix matrix2)
{
    if(matrix1.content==NULL||matrix2.content==NULL)
    {
        printf("The matrix you multiply doesn't exist!\n");
    ...
    {
        if(matrix1.column==matrix2.row&&matrix1.column%8==0&&matrix2.row%8==0)
        {
            float * mpointer=(float *)malloc(4*matrix1.row*matrix2.column);
            long long rowpos=0;
            long long colpos=0;
            long long idx=0;
            for(;colpos<matrix2.column;colpos++)
            {
                for(idx=0;idx<matrix1.column;idx+=8)
                {
                    ...
                    for(rowpos=0;rowpos<matrix1.row;rowpos++)
                    {
                        ...
                        mpointer[rowpos*matrix2.column+colpos]+=(sum[0]+sum[1]+sum[2]+sum[3]+sum[4]+sum[5]+sum[6]+sum[7]);
                    } 
                }
            }
            struct Matrix result={matrix1.row,matrix2.column,mpointer};
            return result;
            free(mpointer);
        }
        else
...
struct Matrix matmul_improved_3(struct Matrix matrix1,struct Matrix matrix2)
{
    if(matrix1.content==NULL||matrix2.content==NULL)
    {
        printf("The matrix you multiply doesn't exist!\n");
    ...
    {
        // if(matrix1.column==matrix2.row&&matrix1.column%16==0&&matrix2.row%16==0)
        // {
        //     float * mpointer=(float *)malloc(4*matrix1.row*matrix2.column);
        //     long long rowpos=0;
        //     long long colpos=0;
        ...
        //             float sum[16]={0};
        //             __m512 mat,mat1;
        //             __m512 mat2=_mm512_set_ps(matrix2.content[colpos+idx*matrix2.column],matrix2.content[colpos+(idx+1)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+2)*matrix2.column],matrix2.content[colpos+(idx+3)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+4)*matrix2.column],matrix2.content[colpos+(idx+5)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+6)*matrix2.column],matrix2.content[colpos+(idx+7)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+8)*matrix2.column],matrix2.content[colpos+(idx+9)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+10)*matrix2.column],matrix2.content[colpos+(idx+11)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+12)*matrix2.column],matrix2.content[colpos+(idx+13)*matrix2.column],\
        //                                 matrix2.content[colpos+(idx+14)*matrix2.column],matrix2.content[colpos+(idx+15)*matrix2.column]);
        //             #pragma omp parallel for
        //             for(rowpos=0;rowpos<matrix1.row;rowpos++)
        //             {
        //                 mat=_mm512_setzero_ps();
        ...
        //                 mpointer[rowpos*matrix2.column+colpos]+=(sum[0]+sum[1]+sum[2]+sum[3]+sum[4]+sum[5]+sum[6]+sum[7]+sum[8]+sum[9]+sum[10]+sum[11]+sum[12]+sum[13]+sum[14]+sum[15]);
        //             } 
        //             idx+=16;
        //         }while(idx<matrix1.column);
        //         colpos++;
        //     }while(colpos<matrix2.column);
        ...
        if(matrix1.column==matrix2.row&&matrix1.column%16==0&&matrix2.row%16==0)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    {
                        mat1=_mm512_loadu_ps(matrix1.content+rowpos*matrix1.column+idx);
                        mat2=_mm512_loadu_ps(matrix2.content+colpos*matrix2.column+idx);
                        mat=_mm512_add_ps(mat,_mm512_mul_ps(mat1,mat2));
                    } 
                    _mm512_storeu_ps(sum,mat);
                    mpointer[rowpos*matrix2.row+colpos]=(sum[0]+sum[1]+sum[2]+sum[3]+sum[4]+sum[5]+sum[6]+sum[7]+sum[8]+sum[9]+sum[10]+sum[11]+sum[12]+sum[13]+sum[14]+sum[15]);
                    colpos++;
                }while(colpos<matrix2.row);
                rowpos++;
            }while(rowpos<matrix1.row);

            struct Matrix result={matrix1.row,matrix2.row,mpointer};
            return result;
            free(mpointer);
        ...
        // if(matrix1.column==matrix2.row&&matrix1.column%16==0&&matrix2.row%16==0)
        // {
        //     long long row1=matrix1.row;
        //     long long col1=matrix1.column;
        //     long long row2=matrix2.row;
        //     long long col2=matrix2.column;
        //     float * m1=matrix1.content;
        //     float * m2=matrix2.content;
        //     float * mpointer=(float *)malloc(4*row1*col2);
        //     long long rowpos=0;
        //     long long colpos=0;
        //     long long idx=0;
        //     do
        ...
        //             float sum[16]={0};
        //             __m512 mat,mat1;
        //             __m512 mat2=_mm512_set_ps(m2[colpos+idx*col2],m2[colpos+(idx+1)*col2],\
        //                                 m2[colpos+(idx+2)*col2],m2[colpos+(idx+3)*col2],\
        //                                 m2[colpos+(idx+4)*col2],m2[colpos+(idx+5)*col2],\
        //                                 m2[colpos+(idx+6)*col2],m2[colpos+(idx+7)*col2],\
        //                                 m2[colpos+(idx+8)*col2],m2[colpos+(idx+9)*col2],\
        //                                 m2[colpos+(idx+10)*col2],m2[colpos+(idx+11)*col2],\
        //                                 m2[colpos+(idx+12)*col2],m2[colpos+(idx+13)*col2],\
        //                                 m2[colpos+(idx+14)*col2],m2[colpos+(idx+15)*col2]);
        //             #pragma omp parallel for
        //             for(rowpos=0;rowpos<row1;rowpos++)
        //             {
        //                 mat=_mm512_setzero_ps();
        //                 mat1=_mm512_loadu_ps(m1+rowpos*col1+idx);
        //                 mat=_mm512_add_ps(mat,_mm512_mul_ps(mat1,mat2));
        //                 _mm512_storeu_ps(sum,mat);
        //                 mpointer[rowpos*col2+colpos]+=(sum[0]+sum[1]+sum[2]+sum[3]+sum[4]+sum[5]+sum[6]+sum[7]+sum[8]+sum[9]+sum[10]+sum[11]+sum[12]+sum[13]+sum[14]+sum[15]);
        //             } 
        //             idx+=16;
        //         }while(idx<col1);
        //         colpos++;
        ...
        //     free(m1);
        //     free(m2);
        //     struct Matrix result={row1,col2,mpointer};
        //     return result;
        //     free(mpointer);
